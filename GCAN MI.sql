...
(
    ...
    FROM MI 
    INNER JOIN PUBLISH.GSCCE.EMPLOYEE_EDV AS REQ_INFO
        ON MI.REQUESTOR_RACFID = REQ_INFO.USERNAME
    INNER JOIN PUBLISH.GSCCE.EMPLOYEE_EDV AS ANALYST_INFO
...
WHERE NOT (ECC_MATERIAL.VALID_FROM - MI_UNION.START_DATE <= 28 AND ECC_MATERIAL.SALES_STATUS IN ('DV', 'DG')); -- remove MI with material that got disco + no stock within 4wk of submisison